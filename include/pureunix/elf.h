...
/* Max envp entries ("KEY=VALUE" strings) elf_exec_argv() will place on a new
 * process's initial stack. */
#define ELF_MAX_ENVP 16
 ...
 * Equivalent to elf_exec_argv(path, 1, (char *const[]){ (char *)path }, NULL). */
 ...
 * instead of uninitialized registers. envp is a NULL-terminated array of
 * "KEY=VALUE" strings (or NULL for an empty environment) copied the same
 * way, so the new process's environ sees real values. See kernel/elf.c's
 * build_argv_stack(). */
 ...
 * userspace-visible equivalent of POSIX execve(). argv/envp follow the same
 * rules as elf_exec_argv(). Only returns (with a negative error code) on
 * failure; regs is the caller's own int $0x80 trap frame, redirected to the
 * new program's entry point on success. */
