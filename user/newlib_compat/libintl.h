...
/* Real, verbatim upstream header (github.com/frida/proxy-libintl, tag
 * 0.5) for the exact libintl.a Meson already auto-vendored as GLib's own
 * 'intl' dependency fallback (see tools/build-glib.sh) — that library's
 * real g_libintl_*() symbols are already linked into every GIO-using
 * PureUnix binary, this was just the one public header piece missing
 * from tools/build-glib.sh's own vendoring step (proxy-libintl doesn't
 * install it to the same /usr/include tree GLib's own headers land in).
 * Placed under newlib_compat/ (not third_party/glib/i686-elf/include/)
 * since any future non-GLib consumer of NEWLIB_CFLAGS gets it for free
 * too, matching this directory's existing "missing newlib header, real
 * implementation already linked" convention (sys/uio.h, mntent.h, ...).
 */
