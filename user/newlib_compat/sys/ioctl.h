 ...
 * user/newlib_syscalls.c — so this isn't just a stub: TIOCGWINSZ/struct
 * winsize match include/pureunix/ioctl.h exactly (the only request
 * PureUNIX's console supports; anything else fails with -EINVAL at
 * runtime, same as the real syscall).
 */
