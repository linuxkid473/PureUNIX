...
yes "" | make oldconfig ARCH=i386 CROSS_COMPILE=i686-elf- >/dev/null
...
make ARCH=i386 CROSS_COMPILE=i686-elf- EXTRA_CFLAGS="${EXTRA_CFLAGS}" || true
...
mapfile -t LINK_OBJS < <(find . -maxdepth 2 \( -name 'built-in.o' -o -name 'lib.a' \) | sort)
