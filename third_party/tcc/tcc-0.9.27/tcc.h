...
/* PureUNIX (github.com — see third_party/tcc/README.md in that tree) has no
 * signal delivery (no SIGSEGV/SIGFPE ever reaches a process), so the
 * -run crash backtrace (CONFIG_TCC_BACKTRACE) and the bounds checker's
 * fault handler (CONFIG_TCC_BCHECK) can never actually fire — and both
 * pull in glibc-shaped <sys/ucontext.h> assumptions (REG_EIP-style
 * macros) this target doesn't have a meaningful equivalent for. Excluded
 * the same way TCC_UCLIBC/TCC_MUSL already exclude CONFIG_TCC_BCHECK
 * below. */
#if defined TCC_IS_NATIVE && !defined CONFIG_TCCBOOT && !defined TCC_PUREUNIX
