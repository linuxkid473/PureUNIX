...
#ifdef TCC_PUREUNIX
    /* PureUNIX has no dynamic linker at all (see third_party/tcc/README.md),
     * so a plain `tcc foo.c` must produce a statically-linked executable by
     * default -- upstream's own default is dynamic (PT_INTERP/PT_DYNAMIC
     * emitted unless -static is passed, see tccelf.c), which would leave
     * unresolved PLT/GOT relocations nothing on this target ever fixes up.
     * Still overridable like any other flag: `-static` is idempotent and
     * there's no `-no-static`/dynamic mode to opt back into on this target. */
    s->static_link = 1;
#endif
