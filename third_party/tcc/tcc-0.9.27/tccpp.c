...
#ifdef TCC_PUREUNIX
/* PureUNIX processes get a fixed, small address-space window (see
 * third_party/tcc/README.md and docs/tcc-port.md) -- upstream's defaults
 * here (768+768+256 KiB = 1.8 MiB, all reserved *before* a single source
 * byte is read) alone exceeded this target's entire heap, making every
 * tcc invocation fail with "memory full (malloc)" immediately in
 * tcc_new()/tccpp_new(). These pools are a pure performance optimization
 * (tal_realloc_impl() falls back to plain tcc_malloc()/tcc_realloc() once
 * a pool is exhausted -- see below -- so shrinking them only costs a few
 * more malloc() calls during heavy compilation, never correctness) --
 * much smaller defaults for a memory-constrained target. */
...
#define CSTR_TAL_SIZE       (32 * 1024) /* allocator for tiny CString instances */
#else
