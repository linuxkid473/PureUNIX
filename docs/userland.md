...
A real [BusyBox](https://busybox.net/) 1.36.1 multi-call binary, cross-compiled with BusyBox's own Kbuild against a deliberately small, non-forking applet set (`third_party/busybox/pureunix.config` — currently `basename`, `cat`, `cp`, `dirname`, `echo`, `false`, `ln`, `ls`, `mkdir`, `mv`, `pwd`, `rm`, `rmdir`, `sleep`, `test`, `touch`, `true`, `wc`, `yes`). BusyBox's own shell (`ash`/`hush`) and anything that forks a pipeline is deferred until PureUNIX has `pipe()`/`dup2()`/real signal delivery — none of the enabled applets need them.
...
The Makefile just copies the vendored ELF into `$(BUILD)/user/busybox.elf` (`$(BUSYBOX_ELF)`, no compilation) and installs it on the EXT2 image only — `tools/mkext2.py`'s `add_bin()` creates a symlink per entry in its `BUSYBOX_APPLETS` list (`/bin/ls -> busybox.elf`, `/bin/cat -> busybox.elf`, ...), exactly like a real BusyBox install's `/bin/ls -> busybox`; BusyBox dispatches on `argv[0]`'s basename (`applets/applets.c`'s `find_applet_by_name()`). FAT16 has no symlinks, so BusyBox isn't installed there.

Building real BusyBox against newlib surfaced a long list of platform gaps — missing headers, functions newlib declares only for specific recognized targets, a genuine `itoa`/`utoa` name collision with newlib's own extensions, and a few syscalls (`SYS_GETUID`/`SYS_GETGID`, `SYS_UTIME`, `SYS_GETTIMEOFDAY`) that didn't exist at all. Every one of them was fixed in the platform layer (`user/newlib_compat/`, `user/newlib_syscalls.c`, new kernel syscalls) rather than by patching BusyBox's own source, per this project's porting philosophy — see `third_party/busybox/README.md`'s "Platform gaps this build needed" section for the full, itemized list rather than duplicating it here.
