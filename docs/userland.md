...
**BusyBox ash is the default login/interactive shell, and BusyBox is the primary userland, as of 2026-07.** A real [BusyBox](https://busybox.net/) 1.36.1 multi-call binary, cross-compiled with BusyBox's own Kbuild. Current applet set (`third_party/busybox/pureunix.config`): `ash`/`sh` (the shell itself — `CONFIG_SH_IS_ASH`), `basename`, `cat`, `chmod`, `chown`, `clear`, `cmp`, `cp`, `cut`, `dd`, `diff`, `dirname`, `echo`, `egrep`, `env`, `false`, `fgrep`, `find`, `grep`, `head`, `hexdump`, `kill`, `less`, `ln`, `ls`, `mkdir`, `mv`, `od`, `paste`, `printenv`, `printf`, `pwd`, `realpath`, `rm`, `rmdir`, `seq`, `sleep`, `sort`, `strings`, `tail`, `tee`, `test`, `touch`, `tr`, `true`, `uniq`, `wc`, `which`, `xargs`, `yes`. `mount`/`umount`/`ps` are deliberately **not** enabled — they need a real `mount(2)` syscall and a `/proc` filesystem, neither of which exists yet (tracked as follow-up work, see project memory, rather than shipped half-working). BusyBox's own `vi` is off too — Neatvi ("`user/vi/` (neatvi)" below) is the preferred full-screen editor, installed as a normal `/bin/neatvi` command. `file` isn't an option at all: BusyBox itself has never implemented a `file(1)` applet (no magic-number database), so there's nothing to enable — not a PureUNIX gap.

`grep`/`egrep`/`fgrep` (and `find -name`'s glob matching, via a real `fnmatch()`) work with real POSIX regex now — see "Regex engine" below. `CONFIG_ASH_BASH_COMPAT` (the `[[ x =~ pattern ]]` operator) is still off, since enabling it pulls in more of ash's own bash-compat machinery than just regex matching and wasn't part of this pass's scope.

Job control (`CONFIG_ASH_JOB_CONTROL`) is off — there's no process-group/session model or `tcsetpgrp()` in this kernel, so `fg`/`bg`/Ctrl-Z suspend don't exist; plain foreground command execution, `&` background jobs, and pipelines all work via ash's own real `fork()`/`pipe()`/`execve()` calls (this kernel's own primitives, unrelated to the in-kernel shell's historical inability to relay data between two *external* pipeline stages — see `docs/shell.md` — since ash is a genuine multi-process program, not a buffer-relay hack).
...
**Source**: `third_party/regex/` (vendored source — see that directory's `README.md`), plus `user/newlib_compat/regex.h` (a current NetBSD `<regex.h>` shadowing newlib's older one).

newlib declares `regcomp()`/`regexec()`/`regerror()`/`regfree()` in `<regex.h>` but ships no implementation for this bare target — the gap that kept `grep`/`egrep`/`fgrep` disabled through earlier phases of this port. Closed by vendoring the classic 4.4BSD/Henry Spencer POSIX regex engine (current NetBSD copy, ~2500 lines, 3-clause BSD license) as real source, compiled by the top-level Makefile (`REGEX_SRCS`/`REGEX_OBJS`) and linked straight into `busybox.elf` by `tools/build-busybox.sh` alongside BusyBox's own objects. Built without `NLS` — no locale/wide-character support, since nothing on PureUNIX ever runs outside the C locale, which is also what lets it avoid needing `mbrtowc()`/real `iswctype()`/etc. See `third_party/regex/README.md` for the exact build configuration and provenance.
...
`user/newlib_syscalls.c`'s `getcwd(NULL, 0)` (the glibc auto-malloc extension ash's own `getpwd()` relies on to seed `$PWD` once at startup) was previously unimplemented (passed the `NULL` straight to the kernel, which rejected it) — the visible symptom was `pwd` printing a blank line until the first `cd`. `realpath()` had the same "declared but not implemented" gap as `dirname()` (see below) — added as a real, from-scratch POSIX canonicalization (resolve `.`/`..`/repeated slashes/symlinks against an absolute base, requiring every component including the last to exist) in `user/newlib_syscalls.c`.

Enabling grep/egrep/fgrep and the rest of this pass's applet set (both real, independent-process testing, not just "it links") surfaced two genuine, previously-undiscovered kernel bugs in the fd layer — dormant since the fd-table rework in an earlier phase, because nothing before now exercised either code path with a *real*, independent multi-process shell:

- **`fcntl(fd, F_DUPFD_CLOEXEC, ...)` wasn't recognized at all**, only plain `F_DUPFD` — `arch/i386/syscall.c`'s `SYS_FCNTL` handler fell through to `-EINVAL` for the numeric value newlib's `<fcntl.h>` actually assigns `F_DUPFD_CLOEXEC` (14, a distinct value from `F_DUPFD`'s 0, not an alias). ash's `savefd()` — called on *every* fd it's about to redirect, including a plain interactive shell's own untouched stdout the very first time it ever runs `cmd > file` — unconditionally uses `F_DUPFD_CLOEXEC` and treats any `fcntl()` failure other than `EBADF` as fatal to the whole interpreter. The practical symptom: any BusyBox ash session died outright (dropped straight back to a fresh login prompt) on its first output redirection, before this pass ever got far enough to test file-consuming applets like `grep`/`cmp`/`diff` at all. Fixed by recognizing `F_DUPFD_CLOEXEC` as an alias for `F_DUPFD` (no close-on-exec model exists here, so there's no behavioral difference to implement) — see `SYS_FCNTL`'s docs below.
- **`open()`/`pipe()`/`dup()`/`fcntl(F_DUPFD)` could never allocate fd 0/1/2**, even after they were explicitly `close()`'d — every allocator started its "lowest free fd" search at index 3 specifically to avoid them. This defeated the standard POSIX `close(0); open(path)` idiom several BusyBox coreutils use for optional-`FILE`-argument handling (`uniq FILE` is the one that surfaced it: with no argument-file handling working, it silently read from the console instead of the file it had just "opened" onto fd 0, and hung waiting for keyboard input). Fixing this properly needed a real way to tell apart three states that all look identical as `used == true, file == NULL` on fd 0/1/2 (never touched; explicitly `close()`'d; or holding a deliberate console binding again after ash's own dup2()-based redirect-then-restore cycle) — see `include/pureunix/task.h`'s `fd_entry_t.closed_explicitly` and `arch/i386/syscall.c`'s `find_free_fd()` for the full reasoning; getting the scope of the fix wrong the first time (treating *any* fd — not just 0/1/2 — with a `NULL` file as reclaimable) reintroduced a *worse* bug where ash's own saved copy of stdout got silently stolen by the next unrelated `open()` call.

Both were caught by, and required extending, the existing `user/systest.c` regression suite (`test_fd_close_reclaims_console_slot()`, deliberately run *last* — see its own comment for why) — not by ad hoc QEMU testing alone.
...
**Source**: `third_party/sqlite/sqlite-3.53.3` (vendored upstream amalgamation — see that directory's `README.md`), built directly by the top-level Makefile's `sqlite3.c`/`shell.c` compile rule against the same newlib + `newlib_syscalls.o` + `newlib_crt0` glue every `NEWLIB_PROGRAMS`/TinyCC/Lua entry uses.

[SQLite](https://www.sqlite.org) 3.53.3 runs natively as `/bin/sqlite3` — the real, unmodified upstream command-line shell, backed by the real upstream SQLite core library (both linked from the same official amalgamation), not a replacement or minimal clone. `-DSQLITE_OS_UNIX=1` selects the real unix VFS backend (`os_unix.c`), which depends on genuine POSIX advisory record locking (`fcntl(F_SETLK/F_GETLK)`) for its PENDING/RESERVED/SHARED/EXCLUSIVE rollback-journal locking protocol — see `include/pureunix/flock.h`/`kernel/flock.c`, a real per-path advisory lock table, not a stub. `-DSQLITE_THREADSAFE=0` (no threading model, same reasoning as Lua's `flockfile()`/`funlockfile()` no-ops) and `-DSQLITE_OMIT_LOAD_EXTENSION` (no dynamic linker, same reasoning as Lua's `LUA_USE_DLOPEN` omission) avoid needing ports this kernel has no model for; `-DSQLITE_OMIT_WAL` disables WAL mode specifically (no shared-memory VFS layer implemented) while leaving the default rollback-journal mode — everything a normal `sqlite3 file.db` session uses — fully real. See `docs/sqlite-port.md` for the full architecture, the real `fcntl()` record-locking implementation, the `SYS_FCHMOD`/`SYS_FCHOWN` syscalls added for journal-permission copying, and — the actual hard part of this port — a genuine, general PureUNIX kernel bug this port found and fixed: a writable, non-append, non-`O_TRUNC` reopen of an existing file used to always start from an empty in-memory buffer, silently discarding real content back to disk at `close()`; SQLite (which always reopens its db file `O_RDWR|O_CREAT` with no `O_TRUNC`) was the first thing in this project to actually depend on that being wrong.

---

## ncurses

**Source**: `third_party/ncurses/ncurses-6.5` (vendored upstream release — see that directory's `README.md`); unlike TCC/Lua/SQLite, the actual build artifact (`third_party/ncurses/i686-elf/`, headers + `libncurses.a`/`libpanel.a`/`libmenu.a`/`libform.a`) is produced once by `tools/build-ncurses.sh` running ncurses' own cross-compiling `configure`/`make` (the same "real generated build output, vendored and committed" pattern `third_party/newlib/` already uses) and then linked by the top-level Makefile's `NCURSES_CFLAGS`/`NCURSES_LDFLAGS`, the same `-isystem`/`-L` shape as `NEWLIB_CFLAGS`/`NEWLIB_LDFLAGS`.

[ncurses](https://invisible-island.net/ncurses/) 6.5 runs natively as `/bin/ncdemo` — a real interactive full-screen TUI application (colors, bordered windows, arrow-key navigation, `SIGWINCH`, clean terminal restoration) linked against the real upstream ncurses library, not a fake curses clone. `--disable-database --with-fallbacks=pureunix,dumb` compiles a PureUNIX-authored terminfo entry (`third_party/ncurses/pureunix.terminfo`, describing `drivers/vga.c`'s ANSI/VT100-subset console exactly, capability by capability) directly into `libncurses.a`, since PureUNIX has no terminfo database on its filesystem at all — every VT's shell gets `TERM=pureunix` (`kernel/main.c`) so this resolves automatically. See `docs/ncurses-port.md` for the full architecture; the console-driver and keyboard-input work this port needed (DECTCEM cursor visibility, relative cursor motion, save/restore cursor, index/reverse-index scrolling, reverse/bold SGR attributes, and — previously silently dropped entirely — arrow/function-key escape sequences reaching `read()` at all); the new `SIGWINCH` signal; and two genuine bugs this port found (an incorrect `@`-cancellation in this port's own terminfo source that crashed `initscr()`, and a real ncurses cross-compilation blind spot — `configure`'s runtime `poll()` probe can't execute cross-compiled, silently disabling escape-sequence recognition for every arrow/function key until worked around).

---

## htop

**Source**: `third_party/htop/htop-3.5.1` (vendored upstream release — see that directory's `README.md`); like TCC/Lua/SQLite (and unlike ncurses), htop's own `configure`/`Makefile.am` only ever *selects* which platform subdirectory's `.c` files join one fixed, hand-enumerable core file list, so it's compiled directly by PureUNIX's own top-level Makefile with a hand-written `third_party/htop/pureunix/config.h` standing in for the one `configure` would otherwise generate.

[htop](https://htop.dev) 3.5.1 runs natively as `/bin/htop` — the real, unmodified upstream interactive process monitor, linked against the real ncurses port above and driving genuinely live PureUNIX process data (real PIDs, states, per-program-varying memory, real delta-computed CPU%, real signal delivery) through `third_party/htop/pureunix/`, a brand-new htop platform backend following the exact same `Platform`/`Machine`/`Process`/`ProcessTable` plugin contract every other htop platform (`linux/`, `freebsd/`, ...) already implements — reading PureUNIX's own real `/proc` (`fs/procfs.c`, already used by BusyBox's own `ps`/`top`), not a fake Linux compatibility shim. See `docs/htop-port.md` for the full architecture; a real kernel-side memory-accounting fix (`task_t.mapped_bytes`, replacing a fixed per-task constant with genuine per-program-varying size); two genuine, general kernel bugs this port found that had nothing to do with htop's own code (`task_t.name`/comm never following `exec()`, and a stale-byte leak in `pack_cmdline()`) — both independently reproduced by BusyBox's own `ps`; a real terminal-compatibility fix (the Enter key now sends the conventional raw `'\r'` byte instead of `'\n'`, needed by any ncurses app that calls `nonl()`); and the small set of general libc gaps (`select()`, `execlp()`, `gethostname()`, a real `dirfd()` stub) this port's build needed. F9 kill was verified end-to-end against a real backgrounded process, confirmed by two independent observers: htop's own zombie-state display and BusyBox ash's own separate job-control termination message.
