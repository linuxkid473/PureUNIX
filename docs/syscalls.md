...
**Write opens** (Stage 4, `flags & O_WRONLY`): if the path doesn't exist and `O_CREAT` is set, `vfs_create()` runs first (a harmless `-EEXIST` from a race is ignored — POSIX `creat()`/`open(O_CREAT)` without `O_EXCL` never errors just because the file already exists). The descriptor's in-memory buffer starts **empty only when `O_TRUNC` is given**; otherwise (including a plain `O_WRONLY|O_CREAT` reopen of an existing file, with no `O_TRUNC` and no `O_APPEND`) it's pre-loaded with the file's existing content via `vfs_read_file()`, exactly like a read open — real POSIX `open()` semantics: `O_WRONLY`/`O_RDWR` alone never discard a file's data, only `O_TRUNC` does. (`O_APPEND` additionally seeks to the end of that pre-loaded content rather than the start.) `SYS_WRITE` then overlays/grows that buffer as data is written; nothing reaches the filesystem until `SYS_CLOSE`. `pu_creat(path)` is `pu_open(path, O_WRONLY|O_CREAT|O_TRUNC)` — this kernel's equivalent of POSIX `creat()`.

Until the SQLite port (`docs/sqlite-port.md`), a non-append write open *always* started from an empty buffer regardless of `O_TRUNC` — every existing writer (ash's `>` redirection, editors' save paths, coreutils) always pairs a writable open with `O_TRUNC` when it wants a blank slate, so this went unnoticed until SQLite's `os_unix.c` — which always reopens its db file `O_RDWR|O_CREAT` with no `O_TRUNC` and depends on seeing prior content across repeated opens of the same file — became the first thing in this project to actually depend on the distinction. Real read-modify-write of an existing file (open once, see existing content, write changes, close) now works with no separate truncating-write step needed.
...
**Limitations**: path pointer is accepted without bounds-checking (no user/kernel memory separation); there is no distinct `O_RDWR` flag value in this kernel's own `O_*` bit layout (`include/pureunix/fcntl.h`) — newlib's `O_WRONLY`/`O_RDWR` both translate to the same single `PU_O_WRONLY` bit (`user/newlib_syscalls.c`'s `open()`), so both are treated identically here. That's harmless now that a non-`O_TRUNC` write open preserves existing content (see above): a real `O_RDWR`-style read-modify-write already works through the same path.
