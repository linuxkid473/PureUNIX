...
- **"Block on I/O" requires the blocking primitive to actually yield**: this only works if whatever a task blocks in calls `task_yield()` in its own wait loop, not just `arch_halt()`. `kernel/vt.c`'s `vt_input_getkey()` (the virtual-terminal keyboard wait — see `docs/vt.md`) and `arch/i386/pit.c`'s `pit_sleep()` both do this (`task_yield(); arch_halt();`, in that order, so an otherwise-idle task still only wakes on the next interrupt rather than spinning). This is what makes a long-lived process left running on a backgrounded virtual terminal (e.g. `ping` on VT2 while VT1 is in use) actually keep making progress: VT1's own idle wait yields to it, and its own sleep/read waits yield right back. Any *other* future blocking primitive needs the same `task_yield()` in its wait loop, or it reintroduces the same "everyone else freezes" failure mode `docs/vt.md`'s concurrency section describes in more detail.
